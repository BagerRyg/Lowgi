...
namespace LowgiHID.HidApi
{
    ...
    internal static partial class HidApi
    {
        ...
        private static nint ResolveNativeLibrary(string libraryName, Assembly assembly, DllImportSearchPath? searchPath)
        {
            ...
            string dllPath = Path.Combine(dllDir, "hidapi.dll");
            if (!File.Exists(dllPath))
            {
                using Stream resource = assembly.GetManifestResourceStream("LowgiHID.libhidapi.hidapi.dll")
                    ?? throw new DllNotFoundException("Embedded hidapi.dll resource was not found.");
                using FileStream file = File.Create(dllPath);
